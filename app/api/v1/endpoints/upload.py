...
def confirm_upload(
    ...
    for idx, row in df.iterrows():
        try:
            raw_description = str(row[payload.mapping["description"]]).strip()
            category_name = categorize(raw_description)

            amount_value = float(row[payload.mapping["amount"]])
            ...
            if type_column and type_column in df.columns:
                ...
                # If the source file only provides a signed amount column,
                # infer direction from the sign and store the magnitude.
                txn_type = TransactionType.SPEND if amount_value < 0 else TransactionType.EARN
